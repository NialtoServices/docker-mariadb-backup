# Docker container for MariaDB Backup

A minimal Alpine Linux container that dumps a MariaDB database on a cron schedule and uploads the backup to a remote storage provider using rclone.
...
1. Dumps the database using `mariadb-dump`, streamed through `zstd` to a `.sql.zst` file
...
| `MARIADB_DB` | | Database name to back up |
...
If your target does not have versioning, set `BACKUP_FILENAME_TIMESTAMPED=true` so each run produces `${MARIADB_DB}-2026-04-17-00-00-00-UTC.sql.zst` instead. Combine with `BACKUP_MAX_VERSIONS=N` to keep only the N most recent and prune older ones after each run. `BACKUP_MAX_VERSIONS` counts runs, not a time window — with a daily schedule and `BACKUP_MAX_VERSIONS=7` you keep a week; with hourly, seven hours.
...
```sh
zstd -dc backup.sql.zst | mariadb -h HOST -u USER -p DB
```
